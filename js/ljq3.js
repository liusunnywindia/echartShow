...
var resizeWorldMapContainer = function () {
	...
	worldMapContainer4.style.height = ((window.innerHeight - 66) * 2 / 3 - 42) + 'px';
	worldMapContainer7.style.height = ((window.innerHeight - 66) / 3 + 30) + 'px';
	worldMapContainer5.style.height = (window.innerHeight - 42) + 'px';
	...
	var tabs = document.getElementById("tabs").offsetWidth;

	if (lilen <= 2) {
		$("#tabs a").css("width", "148px");
	} else {

		var awidth = tabs / lilen + 'px';
		$("#tabs a").css("width", awidth);
	}
...
$(function () {
	$.ajax({
		type : "post",
		async : false,
		url : "http://npubmap.com:8205/AQIService/dp.asmx/GetAirData",
		data : "",
		dataType : "json",
		success : function (json) {
			...
			for (var i = 0; i < geoJson.length; i++) {
				...
				if (geoJson[i].DisplayName != null) {
					air.push(geoJson[i].DisplayName);
					shu.push(geoJson[i].DisplayName, geoJson[i].AQI, geoJson[i].PM25_24H, geoJson[i].PM10_24H, geoJson[i].CO_24H, geoJson[i].NO2_24H, geoJson[i].SO2_24H, geoJson[i].O3_1H_24H, geoJson[i].QUALITY);
					ari.push(shu);
					object = {
						name : geoJson[i].DisplayName,
						type : 'parallel',
						lineStyle : lineStyle,
						data : ari
					};
